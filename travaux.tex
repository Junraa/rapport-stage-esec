...
Comme dit précedemment, un use-after-free est l'enchainement d'une allocation, d'une libération, puis de l'utilisation d'un espace mémoire.
...
\begin{itemize}
    ...
    \item \textbf{pre\_analyze} : Dictionnaire contenant différentes informations sur chacune des fonctions déjà visitées. Optimisation.
    \item \textbf{stack\_arguments} : Arguments placés sur la pile jusque là.
    \item \textbf{marked} : Dictionnaire reliant les noeuds du graphe au nombre d'execution symbolique de ce noeud sur un chemin. Optimisation.
    \item \textbf{loop\_mark} : Dictionnaire reliant un noeud au \textbf{tracking\_set} lors de sa dernière visite. Optimisation.
    \item \textbf{useless\_branches} : Dictionnaire reliant un noeud aux differents branchement qui n'ont aucune conséquence sur l'analyse. Optimisation.
\end{itemize}

\myparagraph{Modification du comportement par default de Miasm}


\myparagraph{Explosion de chemin et Optimisation de boucles}
\myparagraph{Appel de fonctions}
\myparagraph{Pre-Analyse}
\myparagraph{Traitement parralèle ou continu}
\myparagraph{Optimisation Python : \_\_slots\_\_}
